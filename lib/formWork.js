	...
	var helpers = {
		...
		_template : function($object, dataSource, target, mode, trigger){
			...
			if(templateId){
				...
				$(target)[mode](content).trigger(trigger).trigger("liszt:updated");
			}
		},
		_html : function($object, dataSource, target, mode, trigger){
		    $(target)[mode](dataSource).trigger(trigger).trigger("liszt:updated");
		},
		...
		validationError: function ($object, key, message) {
	    ...
	    cnpj :
            {
                ...
                action: function ($) {
                    ...
                    this.on('blur', function () {
                        ...
                        var value = $(this).val().replace(/\W+/gi, '');
                        $(this).val(value);
                        if (value.length === 0) {
                            return;
                        }
                        ...
                        if (!valida_cnpj(value)) {
                            helpers.validationError($(this), 256, "CNPJ Inv&aacute;lido");
                        } else {
                            helpers.validationError($(this), 256);
                        }
	    ...
	    cpf:
            {
                ...
                action: function ($) {
                    ...
                    this.on('blur', function () {
                        ...
                        if (!valida_cpf(value)) {
                            helpers.validationError($(this), 257, "CPF Inv&aacute;lido");
                        } else {
                            helpers.validationError($(this), 257);
                        }
                    }).mask("999.999.999-99");
                }
            },
